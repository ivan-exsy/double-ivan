...
## §1–3 — Diagnostic gap analysis (opener&005 / opener&006)
...
- **Reference stack:** `_style_frame_master.png` (weight 2) primary look + **manual Phaser property top-down** (weight ~1.5, layout) + earlier interior / `cafe_int_counter.png` (weight 1) for cosy continuity.
...
- **Step 0 — manual Phaser top-down (required).** Hand-capture or hand-crop a top-down of the **specific property** and save it under `video/assets/phaser/_moodboard/` (see that folder's `README.md`). **Do not** auto-crop from `1-village-birdeye.png` — auto-trims are too low-res and clip neighbors. Unlabeled crops only (never feed `raw/*_labeled.png` into Imagine). Register the filename in `SECTOR_PHASER_LAYOUT` inside `generate_interior_prompts.py`.
...
- **Step 2 — generate ready-to-paste prompts for the TODO rooms.** Run `python video/assets/scripts-prompts/generate_interior_prompts.py`. It emits `_interior_prompts_TODO.md` — one Grok Imagine prompt per TODO arena, grouped by archetype (bathrooms / kitchens / common rooms / studio living / bedrooms / pub / supply store / market+pharmacy / classroom) so each batch runs in a single Grok session with a locked STYLE block. Bedrooms are persona-neutral by default (literal furniture from the maze, no profession/character flavor) so the same village can host different casts without re-commissioning. Prompts inject the Step-0 Phaser crop as a layout ref (or a **BLOCKED** note if the crop is missing).
- **Step 3 — run the prompts.** Attach: (1) manual Phaser layout, (2) `_style_frame_master.png`, (3) continuity interior. Paste each block into Grok Imagine UI (or the one-shot API helper), save under the suggested name, then update `EXISTING_INTERIORS` in `generate_room_inventory.py` and re-run both scripts — the room flips to DONE and drops out of the TODO prompts file.
...
## v2.4 — Concept-script rewrite changelog
