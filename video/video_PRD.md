...
**P0 — §2.3 Sim-Opening trailer manual asset commissions (do these before switching to other Video PRD tasks).** v0 ships working with placeholders; commissioned drops upgrade quality with ~1h of wiring once assets land. Full prompts, drop-in paths, and acceptance criteria are in **`d:\Coding\double-ivan\20260501_opening-trailer.md`** under each §TODO heading; full filesystem layout is in that doc's "Asset taxonomy" section.

**Asset directory reorg (2026-05-07).** All trailer assets now live under `video/assets/` in five buckets: `phaser/` (auto-captured), `users/` (per-double, including `sketches/`, `headshots/`, `scripts-prompts/`, `character-sheets/`, `sprite-walkouts/`), `village/` (per-village baseline — style frame, exteriors, interiors), `cohort/` (per-cohort hero pairings, how-to-watch cards), `archetypes/` (per-archetype card frames, stings). Capture default in `capture_static_assets.py`, orchestrator path in `generate_trailer.py`, and composer default in `compose_trailer.py` all updated to `video/assets/phaser/`. Composer wiring for non-Phaser commissioned assets (sprite-walkout `.mp4`, card frames, stings, how-to-watch cards) still reads from `assets_dir` (now `phaser/`) — needs path-aware lookup updates when those commissions land; the §TODO entries below call out the per-§TODO LOC delta.
