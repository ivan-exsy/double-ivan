# TODO — LeaderTalks MVP + later backlog
...
>
> **Sequencing (2026-06-11 still):** hardening Phases 2–3 independent. **P1 behaviour before Phase 4 cost** — so we don't tune twice.

---

## Current state (2026-09-16)

**Product MVP = LeaderTalks in Pittsburgh.** Breakfasts was a village / Hobbs rehearsal. Village gather + town talk is a **closed engine gate** — not the product MVP. Do not treat the June “3-sim hardening” spine or this file’s older P1 conversation list as that gate.

**Now live — do not re-open as LeaderTalks blockers**
...
| Watch LIVE catch-up / cliff 1× / scrub / `?double=` | Field PASS on `20260915-3` after `double-front` `e9f2107`. Default LIVE is **6×**; 1× near the live edge. Product choice, not “unlock 1× default” |
...
**Box today (overrides June / Fozzy notes):** Hetzner CX43 `62.238.113.45`. Public API **`https://api.doubland.ai`**. Production `api-gateway` is **TLS on :8001** (plain HTTP = empty reply); do not open `:8001`/`:3000` on the cloud firewall. Unit name is `api-gateway`, not `double-api`. Do not `git checkout main` on the box FE. `PM-LIVE-1` (`sleeping` status) already shipped 2026-06-22. Hostnames `api.ondouble.com` / Fozzy Let's Encrypt are retired.

---

## From village launch (moved 2026-09-03)

Clear later items that lived in `double-ivan/done/20260910_launch.md`. Detail papers stay where they are; this table is the index.

| ID | Item | Status | Notes |
|---|---|---|---|
| **PM-VIL-1** | Seek (everyday + multi) | **Hold** | Own occupancy bet. Satiation is load-bearing. Do not turn seek on to “finish” village. Prior plan (will / linger / seek / sofa / fourth wall): `done/20260825_true_doubles.md` from “Plan: Will, linger, seek”. Linger was later **cut**. **Current (unmute, not this ticket):** already-same 3-part room is a seek **no-op** (no in-cafe `REDIRECT`). Everyday hunt is still Hold. |
| **PM-VIL-2** | “That’s me” / quiz twin | After | Compile quiz/interview into short if–then scene lines; measure express *and* suppress; interview as evidence; human recognition before any public “scientifically you.” **Eval:** person’s own later answer is the ceiling (not 100%); score a **held-out in-village choice** (e.g. stay-and-talk vs leave) — not cafe occupancy, not GSS “85% you.” Park et al. arXiv 2411.10109 is a **survey** twin (GSS/BFI vs two-week retest); it is **not** a village twin. Games in that paper were weaker — expect that band for maze minutes. **Skip:** 2-hour portal interview; Socrates/Centaur as village brain; GUM computer-watching; demographic “act as a [group]”; mixing this gate with Naturalness. **Also:** village IPIP-BFM-25 stem-by-stem sign-off (Week 3 / epic **D6** — still open; portal A2 does not close it). **Charter (living):** `TODO_realism_matriAIx.md`. COS decision: `COS/agents/willwright/kb/wiki/decision/bernstein-believability-vs-accuracy.md`. Research KB (not an implement brief): `done/20260825_true_doubles.md`. Portal A2 stays; this is not a LeaderTalks ship gate. |
| **PM-VIL-3** | Talk Path A (you ↔ one Double) | Open | Talk tab cadence + anti-loop. Village talk does not prove this. **T-E5 smoke PASS** 2026-09-16 (no invented pair; www token live). Owner “yes, I’m your Double” for the rest of Chat → **B1/B2** in `20260917_pre-MVP.md`. **T-W8:** wait copy shipped; engine first-reply latency still here. |
...
| **—** | Daily trailer leftover craft | **LeaderTalks** | `double-ivan/video/TODO_video.md`. Episode 1 closer auto-gen benchmark is locked. Not a village engine gate. Same lane as **PM-BFST-4**. |

Talk leftovers after unmute `20260915-3` (added 2026-09-16). **Not LeaderTalks blockers** unless pulled to `20260917_pre-MVP.md` or marked LeaderTalks below. Explicit go before any of these. Do **not** restore linger or everyday walk-to-person (**PM-VIL-1**). Score card: `double-docs/20260912_Breakfasts/20260915-3_TODOs.md`. Current lock: `double-docs/sot/sot_chats.md` §3c.
...
**Parked village watches (not ranked):** new `session=` after a real sit-stop at cafe; action-text leftover on late walk-ins (re-score only if a resolution rewrite returns). A 4-person talk pass is enough — do not require a 15-person re-score for sit / leftover / unmute.
...
| ID | Item | Status | Notes |
|---|---|---|---|
| **PM-BFST-7** | Allowlist add-one UI (chunk 4b leftover) | After | List + operator API exist. Founder still **inserts** rows. PUT replace stays banned. |
| **PM-BFST-8** | Google OAuth | After | Chat magic-link is **PM-AUTH-1** (pre-MVP). Google button still After. |
| **PM-BFST-9** | Canvas anon `personas_coords` RLS | After | Anon `select` 500 (`42P17` loop). Watch uses RPCs. Only if iframe needs the table. |
...
| **PM-ROUTE-1/2** | `doubland.ai` routing + landing embed sim | Infra | **Partial.** Breakfasts watch URL on www is live (iframe). Remaining **T-W11:** every sim `www.doubland.ai/{id}`; public also `/{SIM_NAME}`. Landing + FE + BE. Not Join. |
...
- [ ] **Conversation quality — Step 3: conversations remember themselves** — recent openers/topics fed into the conversation prompt to break repetition. **Partial (2026-09-16):** leftover clause + retrieved memories + quiet-after-sit exist. That is not a closed pass of “last sit’s topics into the next sit.” Do not treat unmute as Step 3 done.
...
**LangChain harness — steal only** (noted 2026-09-13; the commit that added this wiped the file — restored 2026-09-16). Copy patterns if they help this table (tool routing, graph of steps, checkpoint). Do **not** adopt LangChain / LangGraph as the Doubland runtime; keep our prompt + retrieve + scratch loop.

---

## Owned Double UX (self-serve identity)

- [ ] **PM-OWN-1 — “What my Double knows about me”** — Self-serve surface to read / edit / remove life-chapter notes (`learned` / `currently` / `lifestyle` / `goals`) anytime. Complements consent-based auto-learn from Talk (End session Path A). Not medical/therapy framing. Owner: Ivan (product) + FE when scheduled. **Partial (2026-09-16):** Meet life-notes edit shipped (**T-P4**). ISS PATCH / quiz history / innate regen still BE. Account-anytime surface is not done. **Also (from epic W4-c / Misc):** re-test / profile versioning UI (history data already on BE); after an identity retake, inject a chapter-changed memory so the Double knows the story updated. **S3** post-chat “update your Double” card stays here until B1 write-back / confirm UI (B1 research is `20260917_pre-MVP.md`). Optional later: refresh-snapshot CTA (**PM-BFST-11**).

- [ ] **PM-OWN-2 — “Ask your AI” profile import** — Option to ask an external AI (ChatGPT / Claude / etc.) to build a profile for you, then share/paste that profile into Doubland so your Double can use it. Complements portal quiz/interview and **PM-OWN-1** edit surface. After LeaderTalks. Owner: Ivan (product).

- [ ] **PM-OWN-3 — Daily Dilemma / “what would my Double do?”** — After LeaderTalks retention candidate (epic **W4-b**). Adult dilemma bank **or** a mode on existing Chat-with-Double. Not a second chat product. See After LeaderTalks table.
...
- [ ] **BE-3 — sprite-freeze at 1×** — personas occasionally freeze (assigned target + empty `path` under `BACKEND_INTENT_ONLY_PATH`). **Product (2026-09-12 / field `e9f2107`):** LIVE default is **6×**; 1× only near the live edge / while waiting. Do not treat this ticket as “unlock 1× default.” Still open for freezes visible at the forced 1× cliff. Log `(sim_code, step, persona)` when spotted.
...
- **WATCH — TypeSafe System One / Jev** ([typesafe.ai](https://typesafe.ai/); [intro post ~2026-09-15](https://typesafe.ai/blog/introducing-system-one-models-and-jev)) — typed decisions + calibrated confidence for software; vendor workflow claims ~194× faster / ~445× cheaper vs frontier LLMs on System One tasks *(company evidence, not independent bench)*. **Doubland fit:** hot-path *discrete* engine decisions (talk-initiation, routing, Naturalness/Persona gate scores, confidence act-vs-escalate) — complements skip/zero-call levers when a call must still run. **Not** a replacement for plan/chat/reflect free-text LLMs. **Status: WATCH** — not LeaderTalks / P1 gate; sequencing unchanged (**P1 behaviour before Phase 4 cost**). **Next if pulled:** CTO A/B spike vs current LLM wrappers (latency + $, Naturalness non-regression). Scout: What's New? 2026-09-21 (missed Sep 20 digest aperture; standing Sunday seed added).
...
- **PL-7** — **Rewritten 2026-09-16.** Public API is TLS at `https://api.doubland.ai`. Production `api-gateway` is **TLS on :8001** (plain HTTP = empty reply). Do **not** restore the old Fozzy Let's Encrypt *unit*. Do **not** open `:8001`/`:3000` on the Hetzner Cloud Firewall. Document nginx + localhost topology; do not “fix” by binding the engine to the public internet.
...
- **Village launch post-MVP list** — MOVED 2026-09-03 from `double-ivan/20260910_launch.md` into **PM-VIL-1..5**. Village gather+talk closed the same day. Talk leftovers after unmute `20260915-3` added 2026-09-16 as **PM-VIL-6..11** + **PM-BOOT-1**. **2026-09-16 pull:** T-E8 / T-E5 / PM-BOOT-1 / first-sight / B1–B2 → `20260917_pre-MVP.md`.
