...
- [ ] **PM-OWN-1 — “What my Double knows about me”** — Self-serve surface to read / edit / remove life-chapter notes (`learned` / `currently` / `lifestyle` / `goals`) anytime. Complements consent-based auto-learn from Talk (End session Path A). Not medical/therapy framing. Owner: Ivan (product) + FE when scheduled. **Partial (2026-09-16):** Meet life-notes edit shipped (**T-P4**). ISS PATCH / quiz history / innate regen still BE. Account-anytime surface is not done. **S3** post-chat “update your Double” card stays here until B1 write-back / confirm UI (B1 research is `20260917_pre-MVP.md`). Optional later: refresh-snapshot CTA (**PM-BFST-11**).
