...
> **Updated:** 2026-09-13 (PM-MEM-CTX LangChain note; PM-SEC-7 agent-unreachable backups; PM-SEC-8 door-only security / skip OCR). Village gather + town talk is **closed**. Score trail: `double-ivan/done/20260910_launch.md`. This file is the living post-MVP backlog. Video craft stays in `double-ivan/video/TODO_video.md`. “That’s me” charter stays in `double-ivan/TODO_realism_matriAIx.md`.
>
...
| **PM-SEC-8** | **Door-only security review; skip OCR as process** | Assessed [alibaba/open-code-review](https://github.com/alibaba/open-code-review) 2026-09-13. Skip as CI/process install: no GitHub Action, no `@main` pin, no `pull_request_target` LLM secrets, no Cursor plugin, no merge gate. Built-in Python/TS rules miss Doubland defects (SOT contracts + gateway/RLS/chat). **Do:** one focused pass on the door — FastAPI `api_gateway/` (`:8001`), Supabase RLS, Chat-with-Double. Checklist: auth on create/start/stop/fork vs public status/step; `/docs` in prod; CORS; waitlist rate-limit; service-role never in FE or any agent-readable `.env`; user chat cannot dump other users' threads; published CDN bundles never leak past the watermark. Cheap always-on: `ruff check --select S` on `api_gateway/` only (not whole `reverie/`); `pip-audit` / `npm audit`; GitHub Dependabot. FE: keep eslint; one rule against `dangerouslySetInnerHTML` with chat text. **Steal without the binary:** path-scoped SOT rules in Cloud Agent kickoffs; comments must cite a diff line. **Design/perf stays elsewhere:** PM-MEM-CTX + 20–40 step `analyze_sim` / export verification stats — not a second LLM reviewer. **Trial later only if** Nicolas PRs leak generic bugs Cursor misses: local `ocr review --from main` with Ivan's existing Grok/Anthropic key, project `rule.json` that overrides built-ins with SOT bullets, telemetry off, no CI. High-risk (auth + minors' data) — do not kick off the gateway/RLS/chat pass until Ivan OK. |
...
**PM-SEC-8 note (2026-09-13).** Complements **PM-SEC-4** (public API abuse) and **PM-SEC-5** (write the model down). Do not install a standing PR comment bot. Do not scan the whole sim every PR.
