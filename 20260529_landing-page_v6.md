...
> **What this doc is:** the build spec for the landing-page refresh. **Part A is LOCKED copy + page outline** — implement verbatim. **Part B is the tech plan** for the trailer gallery (YouTube + manual sync script) and keeping the Phaser sim iframe in the hero. **Part C is the visual redesign** (remove the AI-generated look). **Appendix** holds the copy-decision history (reference only).
...
`app/page.tsx` renders, top → bottom (per Locked Copy SOT):

| # | Section | Component file | Status |
|---|---------|----------------|--------|
| 0 | **Metadata** | `app/layout.tsx` | EDIT — copy |
| 1 | **Hero** | `components/hook-section.tsx` | EDIT — new copy + **keep** Phaser sim `<iframe>` + fullscreen logic |
| 2 | **Videos** — trailer gallery | `components/video-section.tsx` | **NEW** |
| 3 | **Explainer** — Get in Doubland in 3 steps | `components/explain-section.tsx` | EDIT — copy |
| 4 | **Amplifier / Social** | `components/amplify-section.tsx` | EDIT — copy |
| 5 | **Safety / Control** | `components/reassure-section.tsx` | EDIT — copy |
| 6 | **Footer** | `components/footer.tsx` | EDIT |

`app/page.tsx` updated import/order:
```tsx
<HookSection />      // hero + Phaser sim iframe + featured trailer player
<VideoSection />     // NEW — trailer gallery (follows hero)
<ExplainSection />
<AmplifySection />
<ReassureSection />
<CtaSection />       // closing CTA (if used)
<Footer />
```
...
### 1. Hero — `hook-section.tsx`

**H1:** `Give your Double a life of its own.`
*H2* '(No real-life consequences. Mostly.)'

**Simulation window** The sim `<iframe>` + fullscreen `postMessage` logic (Phaser sim stays in hero)
...
### 2. Videos — `video-section.tsx` (NEW)

**Header:** `TV show about you and your friends.`

> 🎬 **Hero visual = featured trailer player** (16:9, MVP format). Muted + captioned + poster-framed on load, large play button; click plays with audio. "Press play" is literal. Featured episode follows the **Featuring Rule** (§B2). Shares manifest/state with the §2 gallery — the hero is just the featured slot surfaced up top.
...
### 3. Explainer copy — `explain-section.tsx`
...
### 4. Amplifier / Social — `amplify-section.tsx`
...
### 5. Safety / Control — `reassure-section.tsx`
...
- **Video gallery implementation:** YouTube (source of truth + CDN + player) → **synced manifest** in Supabase → Next.js gallery UI (`video-section.tsx`).
...
- **Phaser sim iframe:** stays integrated in `hook-section.tsx` (Hero). No migration to a separate Live Village section.
...
## B1. Video Gallery — YouTube + synced manifest + manual script
...
Supabase tables `episodes` + `seasons`   ◀── the synced manifest
        │
        ▼  (read at request)
VideoSection (gallery) + HookSection (featured slot)
        │
        ▼
...
sim_id          text       -- "20260520-1"  (links to the sim in hero)
...
`VideoSection` and `HookSection` read from Supabase (server component) using the existing anon key. Read dynamically (Supabase reads are cheap) or cache with `export const revalidate = 300`. Freshness is operator-paced: the gallery reflects whatever the last `pnpm sync:episodes` wrote.
...
## B2. Phaser sim iframe stays in Hero

- The `<iframe src="…/simulations/20260520-1">` block **and** all fullscreen `postMessage`/`fullscreenchange`/Escape logic remain in `hook-section.tsx`.
- Drive the `src` sim id from `seasons.sim_id` so the player and the §2 trailers always reference the same season.
- Add labels: live → `Running now — Day N`; completed → `Replay the full season`. Keep "Open the village ▸" as the expand trigger.
- `VideoSection` handles only the trailer gallery (featured player + rail + ghost cards).

## B3. Implementation checklist

- [ ] Create the worktree/branch first (see Execution rules).
- [ ] `app/layout.tsx` — metadata title/description.
- [ ] `hook-section.tsx` — new H1/subhead/CTAs; **keep** Phaser sim iframe + fullscreen logic; add featured trailer player; secondary CTA anchors to `#videos`.
- [ ] `explain-section.tsx` — 3-step copy.
- [ ] **NEW** `video-section.tsx` — gallery (featured player + rail + ghost cards), reads manifest from Supabase.
- [ ] `amplify-section.tsx`, `reassure-section.tsx`, `cta-section.tsx`, `footer.tsx` — copy.
- [ ] `app/page.tsx` — insert `VideoSection` after `HookSection`; add `CtaSection` if used.
...
- Phaser sim iframe works (incl. fullscreen) in the Hero section; references same `sim_id` as the trailers.
...
> Superseded by Part A (Locked Copy). Retained for traceability of the copy choices. All "Selected" values below have been updated to exactly match the final locked strings.

## A0. Final Copy Decision Table (Current vs Suggested vs Expert vs Selected)

| Landing Page Section | Current (live) | Suggested | Expert | Selected (→ now in Part A Locked Copy) |
|---|---|---|---|---|
...
| **Hero H1** | "Build a Double of You" + "Drop Into a Village With Friends" + "(No real-life consequences. Mostly.)" | `Enter Doubland with friends.` | `Make a Double of yourself. Then let it loose with your friends.` | `Give your Double a life of its own.` |
...
| **Hero Secondary CTA** | None | learn-more | `See how it works` | `Watch the season ▸` (anchors to Videos section) |
...
1. Metadata description — locked (Part A §0). 2. Hero CTA — `Create your Double`, with `Watch the season ▸` secondary (anchors to Videos). 3. Footer tagline — kept `Double your squad, watch the chaos`. 4. Phaser sim iframe stays in Hero (`hook-section.tsx`); trailers live in `video-section.tsx` and reference the same `sim_id`. 5. No separate Live Village section.