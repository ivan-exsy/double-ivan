...
- **Sim jobs thinking OFF (in flight)** — all village LLM jobs on Tier A; Watch Chat stays B / 8,000. Score `20260917-4` @300 vs `20260917-3`. Do not Join. Cap 6.
- **B2 three Chats** on `20260917-3` map Watch (owner / other Double / stranger). No new Start on that sim.
- **Pittsburgh maze** — **PM-BFST-1** occupancy, then **PM-BFST-2** tiles in Supabase, **PM-BFST-3** Join on stamped apartments, **PM-BFST-13** scored PPG Survival day. New Downtown sim — not the Breakfasts watch URL. Cap 6.
...
### LLM thinking remainder (after Day-plan lock)

**This pass (in flight):** every sim job → Tier A (thinking off). Caps unchanged (Day plan 1,200). Watch Chat stays Tier B thinking **high**, cap **8,000**. Score `20260917-4` vs `20260917-3`. Put a job back on B only if that tape is worse.

**Do not** turn Watch Chat thinking off. **Do not** raise sim caps and leave thinking on.
...
**Seen:** named rooms ~3 tiles apart stayed silent (`20260916-1` 0 chats). Current gate is same 3-part room (or 3 tiles on a street with **no** room). **`20260916-2`:** cafe hello Gosha↔Luba ~07:21. **`20260917-3`:** 0 cafe occupancy, 0 village talks — no first-sight chance. **Watch list, not a fail.** Sleeping person stays silent.
