...
7. **Still open** (full execution order in Feedback section): §TODO-S logo splash, §TODO-T iconic flyover, #6 cast-intro grid restructure, #8 Phaser↔rendered fusion beat, #9 dynamic end-card module, #10 two-tier Phaser capture system.
...
- **§TODO-S Doubland logo splash** *(per-app, locked forever)*. ~2–3s static brand mark (wordmark or stylised icon) with a single brand sound (one chime, not a sting). 1280×720 PNG + WAV pair. Drops in as a fixed first beat before any other content. *Effort:* ~2h for the visual + ~30min for the chime + ~10 LOC compose function.
- **§TODO-T Iconic establishing flyover** *(per-app, locked forever)*. ~3–5s signature shot of the_ville — slow zoom-out from the village square revealing landmarks. Reuses the §TODO-N village overhead + a Grok-Imagine video pass. *Effort:* ~1h prompt iteration + xAI render budget. Could reuse one of the existing 5 flyovers; preference is a purpose-cut signature take.
- **§TODO-U Series narrative template** *(per-format, evolves with format)*. Fixed ~15–20s VO that plays over the logo splash + iconic flyover. Template above; only "[N]" (cohort size) and cohort name swap per cohort — everything else is series-level copy. *Effort:* ~1h to lock the script + 1 ElevenLabs render. After lock, idempotent across all future cohorts.
