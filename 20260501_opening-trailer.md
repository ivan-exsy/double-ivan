...
12. **§TODO-VO1 — Closing VO update (added 2026-05-18, revised 2026-05-27):** replace the closing VO `"Day 1 starts now"` with **`"The game is on."`** (locked 2026-05-27). The Episode-1 daily-cadence framing (per `D:\Coding\double-docs\20260519_LIVE_mode.md` DP2 — opening trailer is the user-facing **Premiere**; daily trailers are **Episode 1, 2, 3…** at 18:30 owner-local) moves to the end-card *text* only, e.g. "Episode 1 tomorrow · 18:30". Touches the `habit_hook` system prompt in `showrunner.py` (rewrite examples + intent) and the end-card cadence drawtext in `compose_trailer.py`. Internal/engineering terminology (`day_log`, `--day N`, etc.) is unchanged — user-facing rename only.
...
| Access reveal | 2:10–2:28 | “These are AI Doubles, living in Doubland. The village runs in real time. Watch live. Scroll back to Day 1. Follow any Double.” |
