...
12. **§TODO-VO1 — Closing VO update (added 2026-05-18):** change the closing line from `"Day 1 starts now"` to `"Episode 1 drops tomorrow at 18:30"`. Locked in `D:\Coding\double-docs\20260519_LIVE_mode.md` (DP2 resolution): the opening trailer is now framed user-facing as the **Premiere / Episode 0**; daily trailers become **Episode 1, 2, 3…** at 18:30 owner-local. Touches the Burnett 6-beat narration generator (habit-hook line in `showrunner.py`) and any hardcoded fallback for that VO line. Internal/engineering terminology (`day_log`, `--day N`, etc.) is unchanged — this is a user-facing rename only.
