...
**Status:** Phase 1 FE shipped (2026-04-01) — awaiting BE deployment
**Last updated:** 2026-04-01
**Depends on:** Sprite Detail Card `/card-summary` endpoint (shipped), Chat tab UI (shipped)
...
### Frontend (shipped 2026-04-01)

| File | Action | Purpose |
|---|---|---|
| `components/persona-card/ChatTab.tsx` | **Created** | Full chat UI: message bubbles, suggested-prompt buttons, typing indicator, error handling, thread continuity. Reads `simCode` via `useParams()`, `currentStep` from Redux. |
| `lib/api.ts` | **Modified** | Added `simulationApi.chatWithPersona(simCode, personaName, message, threadId?, step?)` — POSTs to `/chat`, attaches typed `.status` to thrown errors for error classification in ChatTab. |

**FE behaviour notes:**
- `thread_id` persisted in ChatTab local state; follow-up messages reuse it automatically.
- New thread on reopen: ChatTab unmounts when Sprite Card closes (portal destroyed), so `thread_id` resets to `null` — first message of a new session creates a fresh thread.
- 25-message cap: input disabled with "Thread limit reached" placeholder when `thread_message_count >= 25`.
- Rate-limit guard: 400 response containing "rate limit" / "2 threads" maps to user-facing message.
- Suggested prompts sourced from `card_summary.talk.suggested_prompts`; falls back to 4 hardcoded defaults.
...
### Chat tab scrolling / card height (open — 2026-04-01)

**Problem:** The Sprite Card popover has a fixed `maxHeight` driven by viewport size (e.g. ~420px on a 700px-tall screen). After 2–3 exchange turns the message list overflows the available space and the input field is pushed out of view. On mobile (bottom sheet) the input is not pinned — it scrolls with content.

**Root cause:** The CSS flex height chain from the Phaser portal container down to `ChatTab` breaks under certain viewport sizes. `maxHeight` on the outer container does not give descendants a definite height for `h-full` / `flex-1` to work against; the fix applied on 2026-04-01 (`flex-1 min-h-0` on `PopoverContent`'s content wrapper) improves desktop but does not fully resolve mobile, and edge-case viewport sizes may still clip.

**Options to investigate:**
- Switch the popover outer container from `maxHeight` to a fixed `height` (card always occupies its full allocated size; empty space below content on short tabs is acceptable).
- Auto-scale the card height proportionally to conversation length up to a max, then scroll — requires dynamic height recalculation on each new message.
- Mobile: sticky `bottom: 0` positioning on the input row inside the bottom sheet's scroll container.

**Priority:** Polish — the chat is functional for short conversations. Fix before broader rollout.
